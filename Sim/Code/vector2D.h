...
const float MaxFloat = 1000000;//std::numeric_limits<float>::max();
...
typedef std::pair<double, Vector2D> TimeAndPosition;
typedef std::vector<TimeAndPosition> TimedPath;
...
inline float angle(const Vector2D& va, const Vector2D& vb)
{
	...
	// check if frac is out of range (this can happen due to numerical imprecision)
	if (frac < -1 || frac > 1)
		return 0.0f;
